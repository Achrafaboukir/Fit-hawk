...
export default function Menclothes(){
  ...
  useEffect(() => {
    console.log('lfksd')
    axios
      .get('http://127.0.0.1:8000/api/menclothes')
      .then(response => {
        // Check that the response data is an array before setting the state
        if (response.data) {
          setProducts(response.data.menclothes);
  ...
  useEffect(() => {
    console.log('lfksd')
    axios
      .get('http://127.0.0.1:8000/api/menpants')
      .then(response => {
        // Check that the response data is an array before setting the state
        if (response.data) {
          setpants(response.data.menpants);
          console.log(response.data);
  ...
  useEffect(() => {
    axios
      .get('http://127.0.0.1:8000/api/menjacket')
      .then(response => {
        // Check that the response data is an array before setting the state
        if (response.data) {
          setjacket(response.data.menjacket);
          console.log(response.data);
